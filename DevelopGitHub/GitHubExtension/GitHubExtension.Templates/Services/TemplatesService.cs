...
namespace GitHubExtension.Templates.Services
{
    public class TemplatesService : ITemplateService
    {
        ...
        public async Task<HttpResponseMessage> GetIssueTemplateAsync(
            string userName, 
        ...
        public async Task<HttpResponseMessage> GetPullRequestTemplatesAsync(
            string userName, 
            string repositoryName, 
            string pathToFile)
        {
        ...
        private static string GetTemplatesRequestUri(string userName, string repositoryName, string pathToFile)
        {
            var requestUri = string.Format(
                RouteToRepository + "{0}/{1}" + Contents + "{2}", 
                userName, 
                repositoryName, 
                pathToFile);
            return requestUri;
